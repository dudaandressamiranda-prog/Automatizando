// Rode com: npm test (dentro de catalogo-produtos/app)
import test from 'node:test';
import assert from 'node:assert/strict';
import { gtinParaEan, interpretarOcr as interpretar, lerDataDigitada, lerGs1 } from '../src/lib/loteParser.ts';

function caso(texto: string, lote: string | null, validade: string | null) {
...
test("validade digitada à mão", () => {
  assert.deepEqual(lerDataDigitada("5/3/27"), { texto: "05/03/2027", iso: "2027-03-05", soMes: false });
  assert.deepEqual(lerDataDigitada("02/2028"), { texto: "02/2028", iso: "2028-02-29", soMes: true });
  assert.deepEqual(lerDataDigitada("abr 2027"), { texto: "04/2027", iso: "2027-04-30", soMes: true });
...
test("códigos GS1 (DataMatrix / GS1-128) já trazem lote e validade", () => {
  ...
  assert.deepEqual(cru, { gtin: "07891234567895", lote: "AB-123", validade: { texto: "31/03/2027", iso: "2027-03-31", soMes: false } });
  ...
  assert.deepEqual(parenteses!.validade, { texto: "06/2028", iso: "2028-06-30", soMes: true });
