 ...
 *
 * Quando está instalado, os comandos vão direto para a Zebra e o código de
 * barras é desenhado pela impressora, que é o jeito de melhor qualidade — e
 * a etiqueta sai sem passar pela janela de impressão.
 *
...
/**
 * Manda os comandos crus para a impressora. Devolve o erro em texto quando
 * não vai.
 *
 * Cru mesmo: pode ser EPL ou ZPL, e quem escolhe é a tela — a linha Eltron
 * (TLP 2844 e irmãs) só entende EPL, as novas entendem ZPL. Este canal não
 * traduz nem confere nada; mandar a linguagem errada faz o trabalho morrer
 * na impressora sem mensagem.
 */
  ...
  for (const base of ENDERECOS) {
    try {
      const res = await fetch(`${base}/write`, {
        ...
        body: JSON.stringify({ device: impressora, data: comandos }),
